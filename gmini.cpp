...
void updateSystem() {
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        if(verticesHandles[v] != -1) {
            ...
            // WHAT TO PUT HERE ??????? How to update the entries of A ?
            unsigned int x = v * 3;
            unsigned int row = equationIndex * 3;
...
void updateMeshVertexPositionsFromARAPSolver() {
    ...
    unsigned int maxIterationsForArap = 4;
    ...
    for( unsigned int arapIteration = 0 ; arapIteration < maxIterationsForArap ; ++arapIteration ) {
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            if(verticesHandles[v] != -1) {
                ...
                arapLinearSystem.b(row) = mesh.V[v].p[0];
                arapLinearSystem.b(row+1) = mesh.V[v].p[1];
                arapLinearSystem.b(row+2) = mesh.V[v].p[2];
                equationIndex++;
            }
        }
...
void setTagForVerticesInSphere(bool tagToSet)
{
    ...
    Vec3 center; 
    center[0] = sphereSelectionTool.center[0];
    center[1] = sphereSelectionTool.center[1];
    center[2] = sphereSelectionTool.center[2];

    for(unsigned int v = 0;v < mesh.V.size();v++){
        float dist = (mesh.V[v].p - center).length();
...
void updateSphereRadiusWithScroll(int button)
{
    ...
    if (radiusChanged && sphereSelectionTool.isActive) {
...
void draw () {
    ...
    else if (selectionToolState == SelectionTool_Sphere && sphereSelectionTool.isActive) {
        drawSphere(sphereSelectionTool.center[0],
                   sphereSelectionTool.center[1],
                   sphereSelectionTool.center[2],
                   selectionRadius, 20, 20);
    }
}
